 ...
 *  Polihack v3.0
 *  Monitoring System
 *  by Cezar Chirilă, Fodoruţ Ioan, Fărcaş Cirpian, Lupaescu Daniel
 *  
...
uint32_t count = 0;
String status_elder = "ok";
SoftwareSerial mySerial(10, 11); // Bluetooth
...
{  
  ...
  Serial.print("g: ");
  Serial.println(get_g(ax, ay, az));
    
  Serial.print("\nStatus: ");Serial.println(status_elder);
  mySerial.println(status_elder);
  if (status_elder == "ok")
    check_fall(ax, ay, az, gx, gy, gz);
  else if (status_elder == "nk")
    check_rise(ax, ay, az, gx, gy, gz);
}
...
void  check_fall(float ax, float ay, float az, float gx, float gy, float gz)
{
  ...
  if (g < 0.75)
  {
    ...
    while(g < 2.5)
    {
      ...
      g = get_g(ax, ay, az);
      if (millis() - time_fall > 5000)
        break;
    }
    // position: 
    gyro_pos_y = gyro_pos_y / gyro_count;
    gyro_pos_x = gyro_pos_x / gyro_count;
    duration = millis() - time_fall;
    angle_y = duration * abs(gyro_pos_y);
    angle_x = duration * abs(gyro_pos_y);
    Serial.print("gyro_pos_y fall: "); Serial.println(angle_y/1000);
    Serial.print("gyro_pos_x fall: "); Serial.println(angle_y/1000); 
    if ((millis() - time_fall < 5000) && ((angle_y > 20) || (angle_x)))
    {
      status_elder = "nk";
    }
  }
...
void check_rise(float ax, float ay, float az, float gx, float gy, float gz)
{
  double g = get_g(ax, ay, az);
  if (g < 0.85)
    status_elder = "ok";
}
...
{
  // still down
  float time_fall = millis();
  double gyro_pos = 0;
  int     gyro_count = 0;
   while (millis() - time_fall < 5000)
   {
      gyro_pos += gy;
      gyro_count++;
      ...
      gyro_pos = gyro_pos / gyro_count; 
      if ((millis() - time_fall)*abs(gyro_pos) > 40)
      {
        Serial.print("gyro_pos rise: "); Serial.println(gyro_pos);
        status_elder = "up"; 
      }
   }
}
...
float get_g(float ax, float ay, float az)
{
  readAccelData(accelCount);  // Read the x/y/z adc values
  getAres();
